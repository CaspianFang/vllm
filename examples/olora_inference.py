...
def create_test_prompts(lora_path: List[str]) -> List[Tuple[str, SamplingParams]]:
    ...
    return [
        ...
        ("[user] Write a SQL query to answer the question based on the table schema.\n\n context: CREATE TABLE table_name_74 (icao VARCHAR, airport VARCHAR)\n\n question: Name the ICAO for lilongwe international airport [/user] [assistant]",
         ...
         OLoRARequest(LoRA_name_list,lora_int_id=LoRA_id_list,lora_local_path=LoRA_Path_list,self_id = 1, lora_reqs= all_lora_reqs  )),
        ("[user] Write a SQL query to answer the question based on the table schema.\n\n context: CREATE TABLE table_name_11 (nationality VARCHAR, elector VARCHAR)\n\n question: When Anchero Pantaleone was the elector what is under nationality? [/user] [assistant]",
         SamplingParams(n=3,
                        ...
                        max_tokens=128,
                        stop_token_ids=[32003]),
         OLoRARequest(LoRA_name_list[:-1],lora_int_id=LoRA_id_list[:-1],lora_local_path=LoRA_Path_list[:-1],self_id = 2, lora_reqs= all_lora_reqs[:-1])),
        ("[user] Write a SQL query to answer the question based on the table schema.\n\n context: CREATE TABLE table_name_74 (icao VARCHAR, airport VARCHAR)\n\n question: Name the ICAO for lilongwe international airport [/user] [assistant]",
         SamplingParams(temperature=0.0,
                        logprobs=1,
                        prompt_logprobs=1,
        ...
        ("[user] Write a SQL query to answer the question based on the table schema.\n\n context: CREATE TABLE table_name_11 (nationality VARCHAR, elector VARCHAR)\n\n question: When Anchero Pantaleone was the elector what is under nationality? [/user] [assistant]",
         SamplingParams(n=3,
                        ...
                        max_tokens=128,
                        stop_token_ids=[32003]),
         OLoRARequest(LoRA_name_list[0:1],lora_int_id=LoRA_id_list[0:1],lora_local_path=LoRA_Path_list[0:1],self_id = 3, lora_reqs= all_lora_reqs[0:1])),
    ]
