...
# def add_delora_adapter(model: torch.nn.Module, 
#                      delora_path: str,
#                      delora_name: str):
#     delora_config = LoraConfig.from_pretrained(delora_path,
#                                              revision=None,
#                                              use_auth_token=None)
#     key_list = [key for key, _ in model.named_modules()]
...
#         target_module_found = any(
#             re.match(f".*\\.{target_key}$", key)    
#             for target_key in delora_config.target_modules) or any(
#                 target_key == key for target_key in delora_config.target_modules)
            
#         # ==== NEW ====
#         target_module_found = any(
#             re.search(target_key, key) for target_key in delora_config.target_modules) or any(
#                 target_key == key for target_key in delora_config.target_modules)
...
#         print(f"delora target: {target!r}")
        
#         # create and replace
#         _create_and_replace(delora_config, 
#                             delora_name, # "adapter_1"
#                             target,       # ColumnParallelLinear()
#                             target_name,  # qkv_proj
#                             parent)       # LlamaAttention

#     adapters_weights = torch.load(f"{delora_path}/{WEIGHTS_NAME}")
...
#             if "." in suffix:
#                 to_replace = ".".join(suffix.split(".")[1:])
#                 k = k.replace(to_replace, f"{delora_name}.{to_replace}")
#             else:
#                 k = f"{k}.{delora_name}"
#         state_dict[k] = v
