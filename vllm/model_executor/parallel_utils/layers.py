...
# MODIFY
# TODO(xinyu): when delora_name is not in lora_masks, the output is clearly what we want
def compulate_lora(obj: LoraLayer, 
                   ...
                   delora_name: str = None) -> torch.Tensor:
    ...
    for lora_id, lora_mask in lora_masks.items():
        ...
        if lora_id in obj.lora_A.keys():
            ...
            # when delora exists in the request, we need to subtract the delora's result from other loras' output
            if  delora_name != None and lora_id == delora_name:
    ...
    # if delora_name exists and is not in lora_masks, we need to subtract the delora's result from the entire output
    if (delora_name != None)  and (delora_name not in lora_masks.keys()):
        lora_result = obj.scaling[delora_name] * obj.lora_B[delora_name](obj.lora_A[delora_name](x))
        delora_mask = torch.ones_like(next(iter(lora_masks.values())), device=next(iter(lora_masks.values())).device)
        lora_out -= (lora_result * delora_mask.unsqueeze(1).unsqueeze(2))

    return lora_out

# MODIFY
def compute_single_lora(obj: LoraLayer,
                        x: torch.Tensor, 
                        output: torch.Tensor,
                        lora_masks,
                        lora_pos: str,
    ...
    if (delora_name != None) and (delora_name not in lora_masks.keys()):
        ...
        delora_mask = torch.ones_like(next(iter(lora_masks.values())), device=next(iter(lora_masks.values())).device)
        if lora_pos == 'q':
            lora_out[:, :, :qkv_each_size] -= lora_result * delora_mask.unsqueeze(1).unsqueeze(2)
        elif lora_pos == 'k':
            lora_out[:, :, qkv_each_size:2 * qkv_each_size] -= lora_result * delora_mask.unsqueeze(1).unsqueeze(2)
        elif lora_pos == 'v':
            lora_out[:, :, 2 * qkv_each_size:] -= lora_result * delora_mask.unsqueeze(1).unsqueeze(2)
...
class BLoraColumnParallelLinear(ColumnParallelLinear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.delora_name = kwargs.pop('delora_name', None)
    ...
    def forward(self, x: torch.Tensor):
        ...
        lora_out = compulate_lora(self, x, output, self.lora_masks, self.delora_name) # MODIFY
        output += lora_out
...
class QKVLoraLayer(torch.nn.Module):
    ...
    def forward(self, 
        ...
        q_lora_out = compute_single_lora(self.q_lora, x, output, lora_masks, 'q', self.delora_name)
        k_lora_out = compute_single_lora(self.k_lora, x, output, lora_masks, 'k', self.delora_name)
        v_lora_out = compute_single_lora(self.v_lora, x, output, lora_masks, 'v', self.delora_name)
...
class BLoraQKVColumnParallelLinear(ColumnParallelLinear, QKVLoraLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, delora_name)
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, delora_name):
        if self.delora_name != delora_name:
            self.delora_name = delora_name
        ...
        # QUESTION(xinyu): why do we need to update the active adapter?
        self.active_adapter_ = adapter_name
